...
static void *run(void *arg) {
  ...
  int count;
  for (count = 0; 1; count++) {
    struct uffd_msg *uffd_msg = userfaultfd_wait(fd);
...
int main() {
  void *addr = mmap((void *)0x77770000, PAGE_SIZE, PROT_READ | PROT_WRITE,
  ...
  pthread_t thread;
  int err = pthread_create(&thread, NULL, run, (void *)&fd);
  if (err != 0) {
    errno = err;
